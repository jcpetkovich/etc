...
-- dzenCommand = "dzen2 -fn 'xft:Bitstream Vera Sans Mono:size=9' -ta r -p -h 18 -w 789 -x 840"
-- dzenCommand = "dzen2 -fg '#9ACD32' -bg '#333333' -fn '-bitstream-bitstream vera sans mono-medium-*-*-*-11-*-*-0-*-*-iso10646-*' -ta r -p -h 18 -w 789 -x 840"
-- dzenCommand = "dzen2 -e 'onstart=lower' -fg '#9ACD32' -bg '#333333' -fn '-bitstream-bitstream vera sans mono-medium-*-*-*-11-*-*-0-*-*-iso10646-*' -ta l -p -h 18 -w 840"
...
jcKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((modm, xK_p        ), spawn "krunner"                                              ) -- launch krunner
    , ((modm, xK_b        ), sendMessage ToggleStruts                                     ) -- Toggle Struts
    -- , ((modm, xK_left  ), windows W.focusDown                                          ) -- Move focus next
    -- , ((modm, xK_right ), windows W.focusUp                                            ) -- Move focus prev
    , ((modm, xK_f        ), spawn "firefox"                                              ) -- launch firefox
    , ((modm, xK_e        ), spawn "emacsclient -c || (emacs --daemon && emacsclient -c)" ) -- launch emacs
    , ((modm, xK_d        ), spawn "konqueror --profile 'JC profile'"                     ) -- launch konqueror
    , ((modm, xK_c        ), kill                                                         ) -- close window
    , ((modm, xK_q        ), spawn "systemsettings"                                       ) -- launch kde systemsettings
...
-- jcPP h = defaultPP
--              { ppCurrent = wrap "^fg(#)^bg(#09acda) " " ^fg()^bg()"
--              , ppHidden = wrap " " " "
...
--
main = do
  -- xmproc <- spawnPipe "/usr/bin/xmobar /home/jcp/.xmobarrc"
  ...
  xmonad $ kdeConfig
       ...
       -- Print nice stuff for xmobar and move mouse with focus:
